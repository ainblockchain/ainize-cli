...
export function validatePipelineChainConfig(value: PipelineChainConfig): PipelineChainConfig {
  ...
  for (const chain of value.chains) {
    ...
    for (const target of [chain.provider, chain.reader]) {
...
export class PipelineRecorder {
  ...
  constructor(configFile: string, output: string) {
    ...
    const signer = JSON.parse(readFileSync(resolve(this.config.signerFile), 'utf8')) as { others: { private_key: string }[] };
    const privateKey = signer.others?.[this.config.signerIndex]?.private_key;
  ...
  async preflight(): Promise<unknown[]> {
    return Promise.all(this.config.chains.map(async chain => {
      const snapshots = await Promise.all([chain.provider, chain.reader].map(async endpoint => {
  ...
  async record(index: number, runId: string, jobId: string, sequence: number, value: unknown): Promise<{ chain: string; path: string; txHash: string; block: number; valueSha256: string }> {
    ...
    if (existsSync(prefix + '-intent.json')) {
      ...
      assert.equal(await reader.db.ref(reference).getValue({ is_final: true }), null, 'chain path is already occupied');
    ...
    while (Date.now() < deadline) {
      const transaction = await client.getTransactionByHash(intent.hash);
      if (transaction?.is_finalized) {
        ...
        try {
          const remoteValue = await reader.db.ref(reference).getValue({ is_final: true });
          ...
          assert.equal(response.status, 200);
          const blockResponse = await response.json() as { code: number; result: { number: number; hash: string; transactions: (string | { hash: string })[] } };
          ...
          const result = { chain: chain.name, path: reference, txHash: intent.hash, block: blockNumber, valueSha256: createHash('sha256').update(JSON.stringify(value)).digest('hex') };
