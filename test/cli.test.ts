/**
 * The CLI against a seeded in-process node (quorum 1, self-attest allowed so the node lists its own patches).
 *
 * The `agent:` cases that used to live here are in `@ainize/agent`'s own suite now: they were here because this
 * is where somebody had written `startNode` + `seedDemo`, which made this package devDepend on the agent to test
 * the agent — an edge pointing the wrong way. The harness is two exported functions and moved with them.
 */
